...
const REFERRAL_ELIGIBLE_STATUSES = new Set(['Active', 'Reviewing Candidates']);
...
function MessageAthleteButton({
  ...
  const startOutreach = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const apiError = data.error || 'Could not open messages';
        if (/athlete not found/i.test(apiError)) {
          return;
        }
        setError(apiError);
        return;
          ...
          <div className="relative min-h-[156px] px-5 pb-6 pt-0 sm:min-h-[176px] sm:px-8">
            <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
              <div className="flex min-w-0 flex-col gap-4 sm:flex-row sm:items-end">
                ...
                <div className="min-w-0 flex-1 pb-0 pt-1 sm:-mt-20 sm:pt-3 sm:pb-0">
