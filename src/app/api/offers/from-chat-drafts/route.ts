...
/**
 * POST /api/offers/from-chat-drafts — chat-negotiated draft (no campaign mutation).
...
export async function POST(request: NextRequest) {
  ...
  let chatThreadId =
    typeof body.chatThreadId === 'string' ? body.chatThreadId.trim() : '';
  ...
  try {
    ...
    if (isChatSchemaNotReadyError(e)) {
      chatThreadId = await fallbackEnsureOutreachThread(
        session,
        athleteUserId,
  ...
  let offer;
  try {
    offer = await createChatNegotiatedOfferDraft({
      brandUserId: session.id,
      athleteUserId,
      campaignId: campaignId || null,
      notes: contextNote,
